...
BMMGE95.2023.mu    <- 67703  # Kriged commercial biomass.
BMMGE95.2023.sigma <- 4578   # Standard error of kriged commercial biomass.

# Calculate exploitation rate and TAC:
ER  <- harvest.control.rule(BMMGE95.2023.mu, species = 2526)
TAC <- total.allowable.catch(BMMGE95.2023.mu, species = 2526)  # Total allowable catch for sGSL.
...
BREC.2024.mu       <- 37040  #57280 #73120    # Projected R-1 recruitment from the Bayesian projection model.
BREC.2024.sigma    <- 6480   #10540 #14440    # Standard error or projected R-1 recruitment.
...
data <- data.frame(year = 1997:2023,
                   landings = c(17.66, 13.86, 15.52, 19.18, 18.51, 26.18, 21.16, 31.66, 36.08, 29.12, 26.87, 24.46, 23.64, 9.549,
                                10.71, 21.96, 26.05, 24.44, 25.91, 21.71, 43.656, 24.260, 31.707, 28.156, 24.479, 31.661, 35.422),
                   MMGE95SC345.mu = c(27.6882, 28.2949, 31.1769, 9.9793, 17.6121, 13.0600, 26.9933, 21.2590, 23.4963, 19.6210, 26.8285,
                                      ...
                                      20.291, 19.1073, 19.1438, 17.388, 24.393),
...
BMMGE95.2023 <- rlnorm(n, (1 - bias.scs) * BMMGE95.2023.mu, (1 - bias.scs) * BMMGE95.2023.sigma) # Simulate biomass for current year.
BREC.2024    <- rlnorm(n, (1 - bias.rec) * BREC.2024.mu, (1 - bias.rec) * BREC.2024.sigma)       # Projected R-1 mu and sigma.
...
BREM.2024 <- repvec(BMMGE95.2023 * S, ncol = length(catch)) - repvec(catch, nrow = n)

# Projected probability of remaining biomass being below Blim:
Plim <- apply(BREM.2024 < Blim, 2, function(x) sum(x) / length(x))

# Projected probability of total biomass being below Busr:
Pusr <- apply((repvec(BREC.2024, ncol = length(catch)) + BREM.2024) < Busr, 2, function(x) sum(x) / length(x))

# Probability of exceeding ER:
ER.2024 <- repvec(catch, nrow = n) / repvec(BMMGE95.2023, ncol = length(catch))  # Calculate simulated exploitation rates:
PER <- apply(ER.2024 > ER, 2, function(x) sum(x) / length(x))
...
tab <- data.frame(catch = catch,
                  ...
                  P.usr = Pusr,
                  P.flim = PER,
                  mu    = apply(repvec(BREC.2024, ncol = length(catch)) + BREM.2024, 2, mean),
                  lci   = apply(repvec(BREC.2024, ncol = length(catch)) + BREM.2024, 2, quantile, p = 0.025),
                  uci   = apply(repvec(BREC.2024, ncol = length(catch)) + BREM.2024, 2, quantile, p = 0.975))
