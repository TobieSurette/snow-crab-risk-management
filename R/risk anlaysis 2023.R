...
BMMGE95.2022.mu    <- 85532.52  # Kriged commercial biomass.
BMMGE95.2022.sigma <- 5839.20   # Standard error of kriged commercial biomass.

# Calculate exploitation rate and TAC:
ER  <- harvest.control.rule(BMMGE95.2022.mu, species = 2526)
TAC <- total.allowable.catch(BMMGE95.2022.mu, species = 2526)  # Total allowable catch for sGSL.

# Set bias on commericial biomass:
#BMMGE95.2021.mu    <- 67800  # Kriged commercial biomass.
#BMMGE95.2021.sigma <- 7627   # Standard error of kriged commercial biomass

BREC.2023.mu       <- 57280 #73120    # Projected R-1 recruitment from the Bayesian projection model.
BREC.2023.sigma    <- 10540 #14440    # Standard error or projected R-1 recruitment.
...
data <- data.frame(year = 1997:2022,
                   landings = c(17.66, 13.86, 15.52, 19.18, 18.51, 26.18, 21.16, 31.66, 36.08, 29.12, 26.87, 24.46, 23.64, 9.549,
                                10.71, 21.96, 26.05, 24.44, 25.91, 21.71, 43.656, 24.260, 31.707, 28.156, 24.479, 31.682),
...
BMMGE95.2022 <- rlnorm(n, (1 - bias.scs) * BMMGE95.2022.mu, (1 - bias.scs) * BMMGE95.2022.sigma) # Simulate biomass for current year.
BREC.2023    <- rlnorm(n, (1 - bias.rec) * BREC.2023.mu, (1 - bias.rec) * BREC.2023.sigma)       # Projected R-1 mu and sigma.
...
BREM.2023 <- repvec(BMMGE95.2022 * S, ncol = length(catch)) - repvec(catch, nrow = n)

# Projected probability of remaining biomass being below Blim:
Plim <- apply(BREM.2023 < Blim, 2, function(x) sum(x) / length(x))

# Projected probability of total biomass being below Busr:
Pusr <- apply((repvec(BREC.2023, ncol = length(catch)) + BREM.2023) < Busr, 2, function(x) sum(x) / length(x))

# Probability of exceeding ER in 2021:
ER.2023 <- repvec(catch, nrow = n) / repvec(BMMGE95.2022, ncol = length(catch))  # Calculate simulated exploitation rates:
PER <- apply(ER.2023 > ER, 2, function(x) sum(x) / length(x))
...
tab <- data.frame(catch = catch,
                  ...
                  P.usr = Pusr,
                  P.flim = PER,
                  mu    = apply(repvec(BREC.2023, ncol = length(catch)) + BREM.2023, 2, mean),
                  lci   = apply(repvec(BREC.2023, ncol = length(catch)) + BREM.2023, 2, quantile, p = 0.025),
                  uci   = apply(repvec(BREC.2023, ncol = length(catch)) + BREM.2023, 2, quantile, p = 0.975))
...
if (language == "bilingual"){
   ...
   legend.str <- c("Brem/Bres < Blim", "B < Busr/Bnrs", "C")
}

clg()
tiff(file = paste0("results/figures/risk analysis ", year, " - ", language, ".tiff"),
     compression = "lzw", units = "in", res = 300, height = 4.5, width = 7)

plot(range(tab$catch) / 1000, c(0,1), xlab = "", ylab = "",
     cex.lab = 1.5, type = "n", lwd = 2, col = "red", yaxs = "i",
     ylim = c(0, 1.00), cex.axis = 1.0, cex = 0.8, las = 1, xaxs = "i")
grid()
...
legend("topleft", legend = legend.str,
...
mtext(xlab, 1, 2.5, cex = 1.1)
mtext(ylab, 2, 2.5, cex = 1.1)
